...
def get_data(dataset_name, val_ratio, test_ratio, different_new_nodes_between_val_and_test=False,
    ...
    # their edges from training 我们保留作为新节点的样本节点（以测试归纳性），因此我们必须从训练中删除它们的所有边缘
    ...
    train_node_set = set(train_data.sources).union(train_data.destinations)  # union并集符号
    assert len(train_node_set & new_test_node_set) == 0   # &按位和，确保test中均为new node
